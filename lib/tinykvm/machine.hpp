...
struct Machine {
  ...
  using mmap_func_t =
      std::function<void(vCPU &, address_t, size_t, int, int, int, address_t)>;
  ...
  template <typename... Args> void vmcall(const char *, Args &&...);
  template <typename... Args> constexpr void vmcall(address_t, Args &&...);
  /* SYSV function call with timeout */
  template <typename... Args>
  constexpr void timed_vmcall(address_t, float timeout, Args &&...);
  template <typename... Args>
  constexpr void timed_vmcall_stack(address_t, address_t stk, float timeout,
                                    Args &&...);
  ...
  size_t gather_buffers_from_range(size_t cnt, Buffer[], address_t addr,
                                   size_t len) const;
  size_t gather_buffers_from_range(std::vector<Buffer> &, address_t addr,
                                   size_t len) const;
  ...
  size_t writable_buffers_from_range(std::vector<WrBuffer> &, address_t addr,
                                     size_t len);
  /* Lazily create CoW mmap-backed area from an open file descriptor, return the
   * mmap pointer */
  ...
  std::string buffer_to_string(address_t src, size_t len,
                               size_t maxlen = 65535u) const;
  /* Explicitly zero memory range. */
  void memzero(address_t src, size_t len);
  /* View sequential user-writable memory as a string_view, or throw an
     exception. Small structs can be viewed provided the guest over-aligns so
     that it never crosses a page. */
  std::span<uint8_t> writable_memview(address_t src, size_t len);
  /* View sequential user-writable memory as an array of T, or throw an
   * exception. */
  template <typename T>
  T *writable_memarray(address_t src, size_t elements = 1) {
    return reinterpret_cast<T *>(
        writable_memview(src, elements * sizeof(T)).data());
  }
  ...
  void string_or_view(address_t src, size_t size,
                      std::function<void(std::string_view)>,
                      std::function<void(std::string)>) const;

  /* Call function with each segment of memory in given buffer. */
  void foreach_memory(address_t src, size_t size,
                      std::function<void(const std::string_view)>) const;
  /* Efficiently copy between machines */
  void copy_from_machine(address_t dst, Machine &src, address_t sa,
                         size_t size);

  template <typename T> uint64_t stack_push(__u64 &sp, const T &);
  ...
  static void install_syscall_handler(unsigned idx, syscall_t h) {
    m_syscalls.at(idx) = h;
  }
  static void install_unhandled_syscall_handler(numbered_syscall_t h) {
    m_unhandled_syscall = h;
  }
  ...
  template <typename T> T *get_userdata() {
  ...
  address_t max_address() const noexcept {
    return memory.physbase + memory.size;
  }
  ...
  address_t mmap_fixed_allocate(uint64_t addr, size_t bytes, bool is_fixed,
                                int prot = 0x3);
  ...
  uint64_t address_of(std::string_view symbol,
  ...
  void set_printer(printer_func pf = m_default_printer) {
    m_printer = std::move(pf);
  }
  ...
  void print_remote_gdb_backtrace(const std::string &filename,
  ...
  size_t banked_memory_bytes() const noexcept {
    return banked_memory_pages() * vMemory::PageSize();
  }
  /* The extra memory capacity attached to a VM for copy-on-write mechanisms. */
  size_t banked_memory_allocated_pages()
      const noexcept; // How many pages out of the capacity are allocated
                      // (backed by memory)
  size_t banked_memory_allocated_bytes() const noexcept {
    return banked_memory_allocated_pages() * vMemory::PageSize();
  }
  size_t banked_memory_capacity_pages()
      const noexcept; // How many pages is the VM allowed to allocate in total
  size_t banked_memory_capacity_bytes() const noexcept {
    return banked_memory_capacity_pages() * vMemory::PageSize();
  }

  template <typename... Args>
  constexpr void setup_call(tinykvm_x86regs &, uint64_t addr, uint64_t rsp,
                            Args &&...args);
  ...
  void prepare_copy_on_write(size_t max_work_mem = 0,
                             uint64_t shared_memory_boundary = UINT64_MAX,
  ...
  void set_permanent_remote_connection(bool v) {
    m_permanent_remote_connection = v;
  }
  void ipre_remote_resume_now(bool save_all_regs,
                              std::function<void(Machine &)> before);
  ...
  uint32_t remote_connection_count() const noexcept {
    return m_remote_connections;
  }
  ...
  const MachineProfiling *profiling() const noexcept {
  ...
  /// @brief Enable/disable verbose system calls. When enabled, every system
  /// call will be printed to the console, in a trace-like format.
  ...
  /// syscall will be printed to the console, in a trace-like format.
  /// @param verbose True to enable verbose thread syscalls, false to disable
  /// it.
  ...
  void save_snapshot_state_now(const std::vector<std::pair<uint64_t, uint64_t>>
                                   &populate_pages = {}) const;
...
private:
  ...
  void setup_argv(__u64 &, const std::vector<std::string> &,
                  const std::vector<std::string> &);
  void setup_linux(__u64 &, const std::vector<std::string> &,
                   const std::vector<std::string> &);
  void elf_loader(std::string_view binary, const MachineOptions &);
  void elf_load_ph(std::string_view binary, const MachineOptions &,
                   const void *);
  ...
  void
  setup_cow_mode(const Machine *); // After prepare_copy_on_write and forking
  ...
  void remote_pfault_permanent_ipre(uint64_t return_stack,
